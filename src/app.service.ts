...
export class AppService {
  ...
  constructor() {
    this.octokit = new Octokit({ auth: "_" });
    this.git = simpleGit();
  }

  async getPullRequestWithCommits(owner: string, repo: string, pullNumber: number): Promise<any> {
    try {
  ...
  async cloneRepository(owner: string, repo: string, pullNumber: number): Promise<string> {
  ...
  private async runBashScript(clonedDir: string): Promise<void> {
  ...
  async generate(clonedDir: string, pullRequestWithCommits: any) {
    try {
      const prompt = fs.readFileSync(
        path.join(clonedDir, 'script_result', '._output.txt'),
        'utf8',
      );

      const googleAI = new GoogleGenerativeAI(
        'AIzaSyAOotW26evb9nRRoyQmA3GCD562jsLsZ8I',
      );
...
Please ensure your final output clearly outlines your recommendation regarding the pull request's acceptance or rejection, along with detailed rationale and code comments. Here is pul request data: `;
      ...
      console.log(
        'Result written to generated_output.txt in the project root directory.',
      );
